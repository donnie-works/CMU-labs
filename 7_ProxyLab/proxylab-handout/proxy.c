...
void proxy_conn(int clientfd) 
{
    ...
    while ((n = Rio_readnb(&server_rio, buf, MAXLINE)) > 0) {
        Rio_writen(clientfd, buf, n);   // write n bytes in buf to client fd
        if (total + n <= MAX_OBJECT_SIZE) {
            memcpy(cache_buf + total, buf, n); // accumulate each chunk into cache_buf 
        }
        total += n;      // track how many n bytes have been written                           
    }

    /* If response fits, write it to cache */
    if (total <= MAX_OBJECT_SIZE) {
        P(&w);                          // lock cache for writing
        int idx = get_write_idx();      // find empty slot or evict LRU
        cache[idx].hash = request_hash;
        cache[idx].length = total;
        memcpy(cache[idx].object, cache_buf, total);    // copy response into cache slot
        cache[idx].valid = 1;
